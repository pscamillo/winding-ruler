#!/usr/bin/env python3
"""
ruler_generate_v2_1.py — WINDING RULER v2.1: v2 + etiquetagem COS-PONDERADA.

Smoking gun do v1/v2 (registrado no v3.2): raios radiais são INCLINADOS em
relação às folhas da espiral; ∫|∇w|dl infla ~1,8× no caminho inclinado
(evidência: emissão v1 a 10,4 vx vs λ humano 18,76 vx/volta na w2). Os pares
humanos são perpendiculares (cliques folha-a-folha), então o k calibrado
neles subestima a inflação — e o |cos|-weighting (v1.4-B) parecia inútil
porque o teste humano não continha o caso inclinado.

v2.1: integral ∫ g·|n̂·d̂| dl (normais nx/ny decodificadas (raw-128)/127,
nz reconstruído, módulo do dot = imune ao flip por voxel), k recalibrado
com o MESMO estimador. Pares mesma-folha agora integram <0,5 → dw=0 →
rejeitados pelo gate (fonte extra de veneno fechada).
...
def load_channels(dataset, z_begin, z_end, margin_vx=64):
    ...
    for name in ('grad_mag', 'nx', 'ny'):
        ...
        print(f'{name}[4]: shape={subs[name].shape}')
    return subs, z0_full
...
def batched_integrals(subs, z0_full, A, B, chunk=20000):
    """Integral trapezoidal COS-PONDERADA: ∫ g·|n̂·d̂| dl.
    n̂ decodificado dos canais nx/ny ((raw-128)/127, nz=sqrt(1-nx²-ny²)),
    módulo do dot (imune ao flip por voxel). Retorna (integrais, valid)."""
    n = len(A)
    delta = B - A
...
def calibrate_k(dataset, z_begin, z_end):
    ...
    ints, valid = batched_integrals(subs, z0, A, B)
    ints, dws = ints[valid], dws[valid]
    k = float(np.sum(ints * dws) / np.sum(ints * ints))
    print(f'calibração (cos-ponderada): {len(ints)} pares humanos em '
          f'z [{z_begin},{z_end}) → k = {k:.3f}')
    del subs
    return k
...
def main():
    ...
    ap.add_argument('--out', default=os.path.expanduser(
        '~/challenges/vesuvius/spiral-dataset/PHercParis4/'
        'ruler2_1_relative_windings.json'))
    args = ap.parse_args()
    ...
    subs, z0_full = load_channels(args.dataset, args.z_begin, args.z_end)
    ...
    ints, valid = batched_integrals(subs, z0_full, P[ia], P[ib])
    kI = k * ints
    ...
    for j in np.nonzero(accept)[0]:
        ...
        collections[str(cid)] = {
            ...
            'metadata': {'generator': 'winding_ruler_v2_1', 'k': round(k, 4),
    ...
    with open(args.out, 'w') as f:
        json.dump(out, f)
    print(f'\n=== RULER v2.1 (cos): {len(collections)} collections '
